...
export const Advertise = (props) => {
  ...
  return (
    <>
        <hr />
        <div className='grid grid-cols-2 px-8 text-white gap-6 my-10' onClick={scrollToTop}>
            <div>
                <img src={`http://localhost:4000/${props.image}`} alt="" />
            </div>
            <div className='px-2'>
                <p className='text-4xl py-2'>{props.name}</p>
                <p className='text-[#bfb5b5f2] text-sm'>{props.language}</p>
                <p className='text-[#bfb5b5f2] text-sm'>{props.rating}</p>
                <p className='pr-8 my-2'>{props.summary}</p>
